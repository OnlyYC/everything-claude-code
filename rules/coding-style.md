---
name: coding-style
description: Java/Spring Boot 代码风格指南
priority: should
tags: [java, spring-boot, style, lombok]
tech-stack: [Java 21+, Spring Boot 3.x, MyBatis-Plus, Lombok]
---

# 代码风格

> **适用范围**：以下风格指南适用于 **Java / Spring Boot / MyBatis-Plus** 项目
> **技术栈假设**：假设项目使用 Lombok 简化代码。如不使用 Lombok，请手动生成 getter/setter
...
```java
// DTO/VO 使用 Record（Java 21+）或 @Value
public record User(String id, String name, String email) {}

// 或使用 Lombok 不可变注解
@Value  // = @Getter + @FieldDefaults(makeFinal = true, level = PRIVATE)
...
**原则**：
- DTO/VO 优先考虑不可变性（Record 或 @Value）
- Entity 可以保持可变（符合 JPA/MyBatis 规范）
- 跨层传递时返回新对象，避免修改输入参数
...
|------|------|
| **多小文件 > 少大文件** | 高内聚、低耦合 |
| **类大小** | 单个类 150-300 行，最多 500 行 |
| **方法大小** | 单个方法不超过 50 行 |
| **按功能组织** | 按功能/领域组织，而非按类型 |
...
| 异常类型 | 处理策略 | 示例 |
|----------|----------|------|
| **业务异常** | 抛出 BusinessException | 用户不存在、余额不足 |
| **参数异常** | @Valid 自动校验 | 必填字段为空 |
| **系统异常** | 记录日志，返回通用错误码 | 数据库连接失败 |
| **第三方异常** | 降级处理 | 支付网关超时 |
